...
if [ -f "$PUBLIC_KEY" ] && [ -f "$PRIVATE_KEY" ] ; then
    ...
    kubectl -n kube-system create secret tls custom-sealed-secrets-keys --cert="$PUBLIC_KEY" --key="$PRIVATE_KEY"
    kubectl -n kube-system label secret custom-sealed-secrets-keys sealedsecrets.bitnami.com/sealed-secrets-key=active
    kubectl -n kube-system delete pods -l app.kubernetes.io/name=sealed-secrets 
    until $(kubectl -n kube-system logs -l app.kubernetes.io/name=sealed-secrets 2>/dev/null | grep -q "custom-sealed-secrets-keys") ; do
    sleep 1
    echo "Waiting for key pair to be loaded"
    done
else 
    echo "No existing key pair to load."
fi
