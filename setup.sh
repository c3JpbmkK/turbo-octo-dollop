...
do
ARGOCD_INITIAL_ADMIN_SECRET=$(kubectl -n argocd get secrets argocd-initial-admin-secret -o json 2>/dev/null | jq -r '.data.password' | base64 -d || writeError)
...
case $confirm in
    y|Y)
        ...
        argocd cluster add --yes --kubeconfig /home/"${HOST_USER}"/.kube/config ${CURRENT_CONTEXT} 2>/dev/null
        ;;
    n|N)
